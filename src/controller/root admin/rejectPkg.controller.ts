...
const rejectPckage = asyncHandler(async (req: Request, res: Response) => {
  ...
  // check if package is already rejected
  if (TPackage.packageApprovedStatus == "REJECTED") {
    throw new ApiError(400, "Package is already REJECTED");
  }
  ...
  const approvedPackage = await db.bb_travelPackage.update({
    ...
    data: {
      packageApprovedStatus: "REJECTED",
    },
  });
